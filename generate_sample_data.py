#!/usr/bin/env python3
"""
Generate realistic sample benchmark data for demonstration
...
class SampleDataGenerator:
    ...
    def generate_result(self, framework: str, test_name: str, timestamp: datetime) -> BenchmarkResult:
        ...
        response_time = max(50, np.random.normal(
            base_response, 
        ...
        memory_usage = max(100, np.random.normal(
            base_memory,
    ...
    def save_data(self, results: List[BenchmarkResult], format: str = "json"):
        """Save generated data to files"""
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        
        if format == "json":
            filename = f"sample_benchmark_data_{timestamp}.json"
            with open(filename, 'w') as f:
                json.dump([asdict(result) for result in results], f, indent=2)
        
        elif format == "csv":
            filename = f"sample_benchmark_data_{timestamp}.csv"
    ...
    def generate_performance_trends(self) -> Dict[str, Any]:
        ...
        for framework in self.framework_profiles.keys():
            ...
            # Simulate 6 months of weekly averages
            weeks = []
            for week in range(24):  # 6 months
                week_date = datetime.now() - timedelta(weeks=week)
                
                # Add some realistic trends (improvements over time)
                improvement_factor = 1 - (week * 0.005)  # 0.5% improvement per week
                
                avg_response_time = profile["base_response_time"] * improvement_factor
                avg_memory = profile["base_memory"] * improvement_factor
                
                # Add some noise
                avg_response_time += random.uniform(-20, 20)
                avg_memory += random.uniform(-30, 30)
                
                weeks.append({
                    "week": week_date.strftime("%Y-%m-%d"),
                    "avg_response_time_ms": round(max(100, avg_response_time), 1),
                    "avg_memory_usage_mb": round(max(150, avg_memory), 1),
                    "success_rate": min(0.99, profile["success_rate"] + random.uniform(-0.05, 0.05))
                })
            
            trends[framework] = sorted(weeks, key=lambda x: x["week"])
    ...
    def generate_summary_stats(self, results: List[BenchmarkResult]) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "avg_memory_usage_mb": sum(r.memory_usage_mb for r in successful_results) / len(successful_results),
        ...
        for framework in self.framework_profiles.keys():
            ...
            if framework_results:
                framework_stats[framework] = {
                    ...
                    "avg_cost_per_test": sum(r.cost_estimate for r in framework_results) / len(framework_results),
                    "success_rate": len(framework_results) / len([r for r in results if r.framework == framework])
                }
...
def main():
    ...
    generator.save_data(results, "json")
    generator.save_data(results, "csv")
    
    # Generate performance trends
    print("📈 Generating performance trends...")
    trends = generator.generate_performance_trends()
    with open("performance_trends.json", "w") as f:
    ...
    print(f"   📈 {len(trends)} framework trend datasets")
